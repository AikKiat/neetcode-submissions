class Solution:
    ...
    r   3 3 3 3 3 3 3 2 1 1
        ...
        while l <= r and r < len(height) and l < len(height):
            ...
            if r == len(height):
                height[l] = max_right #yeah, basically...what if we chanced upon a wall to the right which is less than the current left wall, but still greater than some of the heights encountered rightwards? Means that we must "lower the boundary", and thus make this current left wall which is too high, perhaps make it to this maximum right height that was found. Then, we can find the small potholes that can fit between lower height values, and bounded by this maximum right height.
                r = l+1
                continue
