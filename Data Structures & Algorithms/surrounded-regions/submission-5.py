            ...
            while queue:
                ...
                if board[curr_r][curr_c] == "O":
                    if 0 < curr_r < len(board)-1 and 0 < curr_c < len(board[0])-1:
                        ...
                        b_surrounded = False #WE DONT immediately return an empty array here upon just finding one bad apple, because we WANT to find ALL others who are connected to the said bad apple, and mark them as visited so we dont cross over them again, PLUS finally at the end of everything THEN return the empty array!! Thus showing that we completely disregard the bunch only when we have discovered all linkages.
