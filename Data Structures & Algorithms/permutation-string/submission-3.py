class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for r in range(len(s2)):
            ...
            #Then, when the value associated to this particular character that exists in s1 drops below zero, we know that beforehand we have removed +1 too much. So, let's shift the left pointer forwards, and letters left behind that were originally considered --> if they belonged to s1 initially or are also = the current character s2[r] which count is < 0 --> add back to the hashmap
