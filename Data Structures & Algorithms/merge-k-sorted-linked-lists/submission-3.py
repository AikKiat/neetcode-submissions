...
class Solution:    
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        for linked_list in lists:
            ...
            index2 = 0
            while current:
                heapq.heappush(min_heap, (current.val, index, index2, current)) #sort by 1st value. However, the same .val means we have to first sort by the order of which linked list came first (index) --> 2nd level of differentiation.
#Then, actually within each linked list we can also have equal .vals --> so 3rd order of differentiation (index2)
