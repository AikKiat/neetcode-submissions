...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node, max_path_sum):

            if not node:
                return 0, max_path_sum
            
            left_result, left_max = dfs(node.left, max_path_sum)
            left_result = max(0, left_result)

            right_result, right_max  = dfs(node.right, max_path_sum)
            right_result = max(0, right_result)

            current_sum = node.val + left_result + right_result
            max_path_sum = max(current_sum, left_max, right_max)
            ...
            return node.val + max(left_result, right_result), max_path_sum

        _, max_path_sum = dfs(root, max_path_sum)
